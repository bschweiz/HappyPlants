    ...
    if (dayWeatherObj.dt_txt.includes("12:00:00")) {
        return (
            <section className="weatherCard">
                <h3 className="weatherCard__day">{new Date(dayWeatherObj.dt * 1000).toLocaleDateString('en-US', { weekday: "short"})}</h3>
                <img className="weatherCard__icon" src={URLAddress} alt={dayWeatherObj.weather[0].description} />
                <h4 className="weatherCard__date">{new Date(dayWeatherObj.dt * 1000).toLocaleDateString('en-US', { month: "numeric", day: "numeric" })}</h4>

    ...
    if (dayWeatherObj.dt_txt.includes("12:00:00")) {
        return (
            <section className="weatherCard">
                <h3 className="weatherCard__date">{new Date(dayWeatherObj.dt * 1000).toLocaleDateString('en-US', { weekday: "short", month: "numeric", day: "numeric" })}</h3>
                <img className="weatherCard__icon" src={URLAddress} alt={dayWeatherObj.weather[0].description} />
                <h4 className="weatherCard__atmosphere">{dayWeatherObj.weather[0].description}</h4>
                {/* <h4 className="weatherCard__windSpeed">Wind Speed: {(dayWeatherObj.wind.speed / 1.609).toFixed(2)}mph</h4> */}
                <h4 className="weatherCard__high">Temp: {((dayWeatherObj.main.temp_max - 273.15) * (9 / 5) + 32).toFixed(1)}&#8457;</h4>
